...
ANON_EMBEDDING_MODEL = "jhgan/ko-sroberta-multitask"
ANON_VECTOR_SIZE = 768
BATCH_SIZE = 100
...
def init_anonymous_patterns(client: QdrantClient, model: SentenceTransformer) -> None:
    ...
    client.create_collection(
        ...
        vectors_config=VectorParams(size=ANON_VECTOR_SIZE, distance=Distance.COSINE),
    )
...
def main() -> None:
    ...
    print(f"SentenceTransformer 모델 로드 중... ({ANON_EMBEDDING_MODEL})", flush=True)
    st_model = SentenceTransformer(ANON_EMBEDDING_MODEL)
    ...
    if not Path(ANONYMOUS_PATTERNS_PATH).exists():
        ...
        init_anonymous_patterns(client, st_model)
